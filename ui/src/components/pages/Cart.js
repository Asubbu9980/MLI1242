...
console.log(val,name,symbol,count)
  ...
  .then(res=>res.text())
  .then(res=>{setDummy(dummy+1);console.log(res)})
...
<Card>
      ...
      return(
        <>
        <Card.Title>{item.symbol}{item.name}<span onClick={()=>{removeItem(data,"name",item.name) }} style={{marginLeft:"190px",fontSize:"15px",color:"red",cursor:"pointer"}}>Remove</span></Card.Title>
      
        <Card.Text>
         cost:<span style={{marginLeft:"320px"}}>₹{item.val}</span>
        </Card.Text>
        <Card.Text>
          {/* Quantity:<span style={{marginLeft:"320px"}}>{item.count}</span> */}
          Quantity:<span style={{marginLeft:"270px"}}>
          <ButtonGroup aria-label="Basic example"  size='sm'>
      <Button variant="danger" onClick={(e)=>{if(count==1){e.preventDefault()} else{count.current-=1;console.log(count.current);handlequantity(item.val,item.name,item.symbol,count.current)}}}>-</Button>
      <Button variant="danger" >{count.current}{console.log(count.current)}</Button>
      <Button variant="danger" onClick={()=>{count.current+=1;console.log(count.current);handlequantity(item.val,item.name,item.symbol,count.current)}}>+</Button>
    </ButtonGroup>
    </span>
        </Card.Text>
        </>
...
{cartitems&&cartitems.map(item=>{
      ...
      return(
        ...
        <Card.Title>{item.symbol}{item.name}<span onClick={()=>{removeItem1(cartitems,"name",item.name)}} style={{marginLeft:"190px",fontSize:"15px",color:"red",cursor:"pointer"}}>Remove</span></Card.Title>
    ...
    <Card style={{marginTop:"10px",}}>
     ...
     <Card.Body>
       <Card.Text style={{borderBottom:"1px dashed grey",paddingBottom:"10px",cursor:"pointer"}} onClick={()=>{navigate("/")}}><i className="fa fa-plus-circle" aria-hidden="true"></i>Add more items<span style={{paddingLeft:"200px"}}><i class="fa fa-arrow-right" aria-hidden="true"></i></span></Card.Text>
       <Card.Text onClick={handleShow} style={{cursor:"pointer"}}><i className="fa fa-pencil-square-o" aria-hidden="true"></i>Add Cooking instructions<span style={{paddingLeft:"135px"}}><i className="fa fa-arrow-right" aria-hidden="true"></i></span></Card.Text>
       <Modal
        show={show}
        onHide={handleClose}
        backdrop="static"
      ...
      >
        ...
        <textarea placeholder="Start typing..." cols={60} rows={4} value={instructions} onChange={(e)=>setInstructions(e.target.value)}></textarea>
    ...
    <Card style={{width:"500px",maxHeight:"250px",marginLeft:"2px",flexBasis:"50%"}}>
...
<Scooter />
    ...
    <Card style={{ width:"500px",maxHeight:"420px",marginLeft:"2px",flexBasis:"50%",marginBottom:"20px" }}>
    <Card.Header style={{textAlign:"center",color:"red"}}>BILL SUMMARY</Card.Header>
      <Card.Body>

      <Card>
       
     {cartitems.length>0&& <Card.Body >
      
        <Card.Text>
       {/* <h6> Subtotal: <span style={{marginLeft:"320px"}}>₹{props.data.val}</span></h6> */}
       <h6> Subtotal: <span style={{marginLeft:"320px"}}>₹{cartitems.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue)}</span></h6>
      ...
      <Card.Text>Delivery Partener fee<span style={{marginLeft:"250px"}}>₹40</span></Card.Text>
        ...
        Grand Total <span style={{marginLeft:"270px",}}>₹{cartitems.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue) +57.85}</span> 
        </Card.Text>
        {coupon&&<Card.Text>Coupon - (TREAT) <span style={{marginLeft:"260px"}}>-₹120</span></Card.Text>}
        {coupon?<Card.Text style={{fontWeight:"bold"}}>Net Payable <span style={{marginLeft:"265px"}}>₹{cartitems.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue) -62.15}</span></Card.Text>:
        <Card.Text style={{fontWeight:"bold"}}>Net Payable <span style={{marginLeft:"265px"}}>₹{cartitems.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue)+57.85}</span></Card.Text>}
      </Card.Body>}
      ...
      {data&& <Card.Body >
       ...
       <h6> Subtotal: <span style={{marginLeft:"320px"}}>₹{data.reduce((total,item) => { return total + item.val * item.count}, initialValue)}</span></h6>
        <p>includes ₹2 feeding India donation</p>
        </Card.Text>
        <Card.Text style={{display:"flex"}}>
          GST and restaurant charges <span style={{marginLeft:"200px"}}>₹17.85</span> </Card.Text>
      <Card.Text>Delivery Partener fee<span style={{marginLeft:"250px"}}>₹40</span></Card.Text>
        ...
        <Card.Text style={{borderTop:"1px dashed grey",marginTop:"-12px",paddingTop:"10px",cursor:"pointer",fontWeight:"bold"}} >
        {/* Grand Total <span style={{marginLeft:"280px",}}>₹{props.data.val +57.85}</span>  */}
        Grand Total <span style={{marginLeft:"270px",}}>₹{data.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue) +57.85}</span> 
        </Card.Text>
        {coupon&&<Card.Text>Coupon - (TREAT) <span style={{marginLeft:"260px"}}>-₹120</span></Card.Text>}
        {coupon?<Card.Text style={{fontWeight:"bold"}}>Net Payable <span style={{marginLeft:"265px"}}>₹{data.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue) -62.15}</span></Card.Text>:
      
      
      <Card.Text style={{fontWeight:"bold"}}>Net Payable <span style={{marginLeft:"265px"}}>₹{net=data.reduce((accumulator,current) => accumulator + current.val*current.count, initialValue)+57.85}</span></Card.Text>}
...
<Card style={{ width:"400px",height:"220px",marginLeft:"2px",flexBasis:"50%",marginTop:"30px",marginBottom:"20px" }}>
    <Card.Header style={{textAlign:"center",color:"red"}}>Pay Using</Card.Header>
      <Card.Body>
        ...
        <input type="radio" name="pay" id="phnpe" value="phonePe" onClick={handleradio} /><npsp> </npsp>
        <label for="phnpe"><span><img src={phone} style={{width:"20px",marginRight:"20px"}} /></span>Phone pe</label><br></br>
       
        <input type="radio" name="pay" id="googlepay" value="googlepay" onClick={handleradio}/><npsp> </npsp>
        ...
        <label for="upi"><span><img src={upi} style={{width:"30px",marginRight:"20px"}} onClick={handleradio}/></span>UPI</label><br></br>

        <input type="radio" name="pay" id="net" value="net" /><npsp> </npsp>
        <label for="net"><span><img src={netb} style={{width:"20px",marginRight:"20px"}} onClick={handleradio}/></span>Net Banking</label><br></br>

        <input type="radio" name="pay" id="cod" value="cod" /><npsp> </npsp>
        <label for="cod"><span><img src={cod} style={{width:"20px",marginRight:"20px"}} onClick={handleradio}/></span>Cash on delivery</label>

       </form>
</Card.Body>
</Card>
<button type='submit' style={{marginLeft:"40px",color:"red"}} onClick={handleClickOpen}>Confirm details</button>
{localStorage.getItem("token")?<button type='submit' style={{marginLeft:"40px",color:"red"}} onClick={handlePayment}>Complete Payment</button>:<button type='submit' style={{marginLeft:"130px",color:"red"}} onClick={()=>{alert("Please Login first to complete payment");navigate("/")}}>Complete Payment</button>}
   {/* handlePayment */}
<Dialog
        fullScreen
        open={open}
      ...
      >
        <AppBar sx={{ position: 'relative',backgroundColor:"aliceblue",color:"black"}}>
          <Toolbar>
            <IconButton
              edge="start"
              color="inherit"
            ...
            <Button autoFocus color="inherit" onClick={()=>{if(!name || !address || !phn){
          ...
          <ListItem button>
            <ListItemText
              ...
              secondary={<card> <Card.Body>
                <Card.Text style={{marginTop:"10px"}}>Name:{name}</Card.Text>
                <Card.Text>Address:{address}</Card.Text>
                <Card.Text>Phone number:{phn}</Card.Text>
              </Card.Body></card>}
            />
